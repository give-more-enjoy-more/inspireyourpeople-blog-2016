<header class="gmv3-header">
	<div class="info-account-bar">
		<div class="inner-container">
			
			<p class="contact-info">
				Call anytime. 1-866-952-4483<br />
				<span class="overnight-shipping">Overnight shipping available.</span>
			</p>

			<div class="current-customer-info">
				
				<p class="my-account">
					<a href="http://www.inspireyourpeople.com/login/">Sign In</a> / <a href="http://www.inspireyourpeople.com/login/">Sign Up</a>
				</p>

				<div class="search-form-container">
					<form class="search-form" method="get" action="<?phpphp echo esc_url( home_url( '/' ) ); ?>">
						<p>
							<input name="s" id="s" type="text" placeholder="Search The GiveMore Blog..." value=""/>
							<input class="sml-btn" type="submit" class="search-submit" value="Search" />
					  </p>
					</form>
				</div> <?php /* END .search-form-container */ ?>

			</div> <?php /* END .current-customer-info */ ?>
		</div> <?php /* END .inner-conatiner */ ?>
	</div> <?php /* END .info-account-bar */ ?>

	<div class="top-navigation">
		<div class="inner-container">
			
			<a href="http://www.inspireyourpeople.com/"><img class="iyp-logo" src="http://www.inspireyourpeople.com/themes/gmv3/resources/images/logos/header/inspireyourpeople-logo-212x45.png" alt="InspireYourPeople.com" width="212" height="45" /></a>

			<ul class="top-navigation-menu-items">
				
				<li class="menu-item">
					<a href="/blog/sams-thoughts/" class="menu-item-link avoid-follow-if-touch-device">Blog</a>
					
					<div class="mini-submenu right-submenu">
						<ul>
							<li><a href="/blog/sams-thoughts/">Sam's Thoughts</a></li>
							<li><a href="/blog/quotes/">Quotes (motivating &amp; thoughtful)</a></li>
							<li><a href="/blog/tools/">Tools (useful stuff)</a></li>
							<li><a href="/blog/videos/">Videos (inspiring &amp; moving)</a></li>
							<li><a href="/blog/subscribe/">Subscribe</a></li>
						</ul>
					</div> <?php // END .mini-submenu .right-submenu ?>					
				</li> <?php // END .menu-item - Blog ?>


				<li class="menu-item">	
					<a href="/who-we-are/" class="menu-item-link avoid-follow-if-touch-device">About Us</a>

					<div class="mini-submenu right-submenu">
						<ul>
							<li><a href="/who-we-are/">Who We Are</a></li>
							<li><a href="/connect-with-sam/">Connect With Sam</a></li>
							<li><a href="/customers/">Our Customers</a></li>
							<li><a href="/customer-buzz/">Customer Feedback</a></li>
							<li><a href="/books-we-love/">Books We Love</a></li>
							<li><a href="/blog/subscribe/">Subscribe</a></li>
							<li><a href="/work-with-us/">Work With Us</a></li>
							<li><a href="/support/contact/">Contact Us</a></li>
						</ul>
					</div> <?php // END .mini-submenu .right-submenu ?>
				</li> <?php // END .menu-item - About Us ?>


				<li class="menu-item">
					<a href="/speaking/" class="menu-item-link">Speaking</a>
				</li> <?php // END .menu-item - Speaking ?>
			
			
				<li class="menu-item">
					<a href="#" class="menu-item-link avoid-follow-if-touch-device">Schools</a>
					
					<div class="mini-submenu center-submenu">
						<div class="submenu-column">
							<p class="mini-submenu-title">Ways to use</p>
							<ul>
								<li><a href="/schools/">K-12 Schools</a></li>
								<li><a href="/orientation/">College Orientation</a></li>
								<li><a href="/speaking/">Convocation Speaker</a></li>
							</ul>
						</div>

						<div class="submenu-column last">
							<p class="mini-submenu-title">School Themes</p>
							<ul>
								<li><a href="/212-the-extra-degree-in-schools/">212 the extra degree</a></li>
								<li><a href="/smile-and-move-in-schools/">Smile &amp; Move</a></li>
								<li><a href="/cross-the-line-in-schools/">Cross The Line</a></li>
								<li><a href="/love-your-people-in-schools/">Love Your People</a></li>
								<li><a href="/lead-simply-in-schools/">Lead Simply</a></li>
							</ul>
						</div>
					</div> <?php // END .mini-submenu center-submenu ?>
				</li> <?php // END .menu-item - Education ?>
				
				
				<li class="menu-item">
					<a href="#" class="menu-item-link avoid-follow-if-touch-device">Ways to Use</a>
					
					<div class="mini-submenu">
						<ul>
							<li><a href="/meetings-discussions/">Use With a Group</a></li>
							<li><a href="/sales-meetings/">Sales Meetings</a></li>
							<li><a href="/retail-holiday-season/">Holiday Retail Training</a></li>
							<li><a href="/212-golf-meeting/">212 Golf Meeting</a></li>
							<li><a href="/run-better-meetings/">How to Run Better Meetings</a></li>
						</ul>
					</div> <?php // END .mini-submenu ?>
				</li> <?php // END .menu-item - Ways to Use ?>
			
			
				<li class="menu-item">
					<a href="#" class="menu-item-link avoid-follow-if-touch-device">Our Messages</a>
					
					<div class="submenu">
						<div class="inner-container brand-submenu-container">
							<ul class="submenu-brand-list">
								
								<li class="submenu-brand-list-item">
									<a class="reveal-brand-info-submenu avoid-follow-if-touch-device" href="/212-the-extra-degree/">212 the extra degree</a>
									
									<div class="brand-info-submenu">
										<a class="nav-brand-teaser" href="/212-the-extra-degree/"><img src="http://www.inspireyourpeople.com//themes/gmv3/resources/images/throughout/navigation/navigation-brand-212-245x155.png" alt="212 the extra degree" width="245" height="155" /></a>
									
										<p class="nav-brand-description">212 the extra degree can help you inspire people to embrace the value of <strong class="tt-color">extra effort and attention</strong> that goes into creating great results.</p>
										<p class="nav-brand-cta-btns"><a class="flat-btn tt-btn" href="/212-the-extra-degree/">Learn about 212&deg;</a> <a class="flat-btn flat-outline-btn tt-btn" href="/212-the-extra-degree-packages/">See packages</a> <a class="flat-btn flat-outline-btn tt-btn" href="/products/212-the-extra-degree/">Shop all</a></p>
									</div> <?php // END .brand-info-submenu ?>
								</li> <?php // END .submenu-brand-list-item - 212 ?>

								<li class="submenu-brand-list-item">
									<a class="reveal-brand-info-submenu avoid-follow-if-touch-device" href="/smile-and-move/">Smile &amp; Move</a>

									<div class="brand-info-submenu">
										<a class="nav-brand-teaser" href="/smile-and-move/"><img src="http://www.inspireyourpeople.com//themes/gmv3/resources/images/throughout/navigation/navigation-brand-sm-245x155.png" alt="Smile &amp; Move" width="245" height="155" /></a>

										<p class="nav-brand-description">Smile &amp; Move can help you encourage people to be more <strong class="sm-color">positive and service-oriented</strong> in their efforts with customers and colleagues.</p>
										<p class="nav-brand-cta-btns"><a class="flat-btn sm-btn" href="/smile-and-move/">Learn about Smile &amp; Move</a> <a class="flat-btn flat-outline-btn sm-btn" href="/smile-and-move-packages/">See packages</a> <a class="flat-btn flat-outline-btn sm-btn" href="/products/smile-and-move/">Shop all</a></p>
									</div> <?php // END .brand-info-submenu ?>							
								</li> <?php // END .submenu-brand-list-item - SM ?>
								
								<li class="submenu-brand-list-item">
									<a class="reveal-brand-info-submenu avoid-follow-if-touch-device" href="/cross-the-line/">Cross The Line</a>

									<div class="brand-info-submenu">
										<a class="nav-brand-teaser" href="/cross-the-line/"><img src="http://www.inspireyourpeople.com//themes/gmv3/resources/images/throughout/navigation/navigation-brand-ctl-245x155.png" alt="Cross The Line" width="245" height="155" /></a>

										<p class="nav-brand-description">Cross The Line can help you <strong class="ctl-color">encourage</strong> people to <strong class="ctl-color">commit</strong> to going beyond <strong class="ctl-color">mediocrity</strong> in everything they do.</p>
										<p class="nav-brand-cta-btns"><a class="flat-btn ctl-btn" href="/cross-the-line/">Learn about Cross The Line</a> <a class="flat-btn flat-outline-btn ctl-btn" href="/cross-the-line-packages/">See packages</a> <a class="flat-btn flat-outline-btn ctl-btn" href="/products/cross-the-line/">Shop all</a></p>
									</div> <?php // END .brand-info-submenu ?>							
								</li> <?php // END .submenu-brand-list-item - CTL ?>
								
								<li class="submenu-brand-list-item">
									<a class="reveal-brand-info-submenu avoid-follow-if-touch-device" href="/love-your-people/">Love Your People</a>

									<div class="brand-info-submenu">
										<a class="nav-brand-teaser" href="/love-your-people/"><img src="http://www.inspireyourpeople.com//themes/gmv3/resources/images/throughout/navigation/navigation-brand-lyp-245x155.png" alt="Love Your People" width="245" height="155" /></a>

										<p class="nav-brand-description">Love Your People can help you<br />encourage people to be <strong class="lyp-color">kinder</strong> and<br /><strong class="lyp-color">more accountable</strong> to each other and<br />the people they serve.</p>
										<p class="nav-brand-cta-btns"><a class="flat-btn lyp-btn" href="/love-your-people/">Learn about Love Your People</a> <a class="flat-btn flat-outline-btn lyp-btn" href="/love-your-people-packages/">See packages</a> <a class="flat-btn flat-outline-btn lyp-btn" href="/products/love-your-people/">Shop all</a></p>
									</div> <?php // END .brand-info-submenu ?>							
								</li> <?php // END .submenu-brand-list-item - LYP ?>
								
								<li class="submenu-brand-list-item">
									<a class="reveal-brand-info-submenu avoid-follow-if-touch-device" href="/lead-simply/">Lead Simply</a>

									<div class="brand-info-submenu">
										<a class="nav-brand-teaser" href="/lead-simply/"><img src="http://www.inspireyourpeople.com//themes/gmv3/resources/images/throughout/navigation/navigation-brand-ls-245x155.png" alt="Lead [simply]" width="245" height="155" /></a>
										
										<p class="nav-brand-description">Lead Simply can help you inspire<br />leaders to focus on what's most important (and often missed) in creating a <strong class="ls-color">special team of people.</strong></p>
										<p class="nav-brand-cta-btns"><a class="flat-btn ls-btn" href="/lead-simply/">Learn about Lead Simply</a> <a class="flat-btn flat-outline-btn ls-btn" href="/lead-simply-packages/">See packages</a> <a class="flat-btn flat-outline-btn ls-btn" href="/products/lead-simply/">Shop all</a></p>
									</div> <?php // END .brand-info-submenu ?>							
								</li> <?php // END .submenu-brand-list-item - LS ?>
								
								<li class="submenu-brand-list-item">
									<a class="reveal-brand-info-submenu avoid-follow-if-touch-device" href="/sales-tough/">SalesTough</a>

									<div class="brand-info-submenu">
										<a class="nav-brand-teaser" href="/sales-tough/"><img src="http://www.inspireyourpeople.com//themes/gmv3/resources/images/throughout/navigation/navigation-brand-st-245x155.png" alt="SalesTough" width="245" height="155" /></a>

										<p class="nav-brand-description">SalesTough can help you create a sales team that's <strong class="st-color">focused, resilient, and results-oriented.</strong></p>
										<p class="nav-brand-cta-btns"><a class="flat-btn st-btn" href="/sales-tough/">Learn about SalesTough</a> <a class="flat-btn flat-outline-btn st-btn" href="/salestough-packages/">See packages</a> <a class="flat-btn flat-outline-btn st-btn" href="/products/salestough/">Shop all</a></p>
									</div> <?php // END .brand-info-submenu ?>							
								</li> <?php // END .submenu-brand-list-item - ST ?>
								
								<li class="submenu-brand-list-item">
									<a class="reveal-brand-info-submenu avoid-follow-if-touch-device" href="#">More Messages</a>

									<div class="other-brands-submenu">
										<ul class="submenu-column">					
											<li><a href="/discover/be-no-ego/">Be No Ego</a></li>
											<li><a href="/discover/complain-less/">ComplainLess</a></li>
											<li><a href="/discover/be-resilient/">Be Resilient</a></li>
											<li><a href="/discover/push-your-luck/">Push Your Luck</a></li>
											<li><a href="/discover/no-gomos/">No Gomos</a></li>
										</ul>
										
										<ul class="submenu-column">
											<li><a href="/discover/no-excuses-drama-complaints/">No Excuses, Drama, Complaints</a></li>
											<li><a href="/category/make-good-things-happen/">Make Good Things Happen</a></li>
											<li><a href="/discover/give-take/">Give &gt; Take</a></li>
											<li><a href="/discover/listen-talk/">Listen &gt; Talk</a></li>
											<li><a href="/category/declaration-of-contribution/">Declaration of Contribution</a></li>
										</ul>
									</div> <?php // END .other-brands-submenu ?>
								</li> <?php // END .submenu-brand-list-item - More Brands ?>

							</ul> <?php // END .submenu-brand-list ?>
						</div> <?php // END .inner-container ?>
					</div> <?php // END .submenu ?>
				</li> <?php // END .menu-item - Our Messages ?>

				
				<li class="menu-item">
					<a href="/shop/" class="menu-item-link avoid-follow-if-touch-device">Shop</a>
					
					<div class="submenu">
						<div class="inner-container">

							<div class="submenu-products">
								<p class="submenu-title">Products</p>
								<ul class="submenu-column">
									<li><a href="/category/books/">Books</a></li>
									<li><a href="/category/combo-packs-by-sam-parker/">Combo Packs</a></li>
									<li><a href="/category/videos/">Videos</a></li>
									<li><a href="/category/presentations/">Presentations</a></li>
									<li><a href="/compare-meeting-packages/">Meeting Packages</a></li>
									<li><a href="/category/posters-and-prints/">Posters &amp; Prints</a></li>
									<li><a href="/category/banners/">Banners</a></li>
									<li><a href="/schools/">Schools</a></li>
								</ul>
								
								<ul class="submenu-column">
									<li><a href="/category/wristbands/">Wristbands</a></li>
									<li><a href="/category/pocket-cards/">Pocket Cards</a></li>
									<li><a href="/category/cards-and-stationery/">Cards &amp; Stationery</a></li>
									<li><a href="/category/notepads-pens-and-mouse-pads/">Notepads, Pens, &amp; Mouse Pads</a></li>
									<li><a href="/category/mugs-and-water-bottles/">Mugs &amp; Water Bottles</a></li>
									<li><a href="/category/recognition-and-gifts/">Recognition &amp; Gifts</a></li>
									<li><a href="/category/stickers-and-magnets/">Stickers &amp; Magnets</a></li>
									<li><a href="/category/apparel/">Apparel</a></li>
								</ul>
							</div> <?php // END .submenu-products ?>
		
							<div class="submenu-brands">
								<p class="submenu-title">Our Messages</p>
								<ul class="submenu-column">
									<li><a href="/products/212-the-extra-degree/">212 the extra degree</a></li>
									<li><a href="/products/smile-and-move/">Smile &amp; Move</a></li>
									<li><a href="/products/cross-the-line/">Cross The Line</a></li>
									<li><a href="/products/love-your-people/">Love Your People</a></li>
									<li><a href="/products/lead-simply/">Lead Simply</a></li>
									<li><a href="/products/salestough/">SalesTough</a></li>
									<li><a href="/discover/be-no-ego/">Be No Ego</a></li>
									<li><a href="/discover/complain-less/">ComplainLess</a></li>
								</ul>
								
								<ul class="submenu-column">
									<li><a href="/discover/no-excuses-drama-complaints/">No Excuses, Drama, Complaints</a></li>
									<li><a href="/category/make-good-things-happen/">Make Good Things Happen</a></li>
									<li><a href="/discover/be-resilient/">Be Resilient</a></li>
									<li><a href="/discover/push-your-luck/">Push Your Luck</a></li>
									<li><a href="/discover/no-gomos/">No Gomos</a></li>
									<li><a href="/discover/give-take/">Give &gt; Take</a></li>
									<li><a href="/discover/listen-talk/">Listen &gt; Talk</a></li>
									<li><a href="/category/declaration-of-contribution/">Declaration of Contribution</a></li>
								</ul>
							</div> <?php // END .submenu-brands ?>
							
						</div> <?php // END .inner-container ?>
					</div> <?php // END .submenu ?>
				</li> <?php // END .menu-item - Shop ?>
			
			</ul> <?php // END .top-navigation-menu-items ?>

		</div> <?php // END .inner-container ?>
	</div> <?php /* END .top-navigation */ ?>
</header>